...
var toolManuals = []ToolManual{
	...
	{
		Name: toolSearch, Purpose: "Discover short-lived Kubernetes capability handles allowed by server policy and the caller's Kubernetes RBAC.", Modes: allAccessModes,
		Inputs: []ToolManualInput{
			{Name: "query", Description: "Optional kind, resource, API group, category, or action text."},
			{Name: "action", Description: "Optional exact logical action or Kubernetes verb filter."},
			{Name: "exactKind", Description: "Optional exact kind filter, for example Pod."},
			{Name: "exactResource", Description: "Optional exact plural resource or resource/subresource filter, for example pods or pods/log."},
			{Name: "apiGroup", Description: "Optional exact API group; use core for the core API group."},
			{Name: "version", Description: "Optional exact API version, for example v1."},
		...
		Returns: []string{"Authorized capability summaries with compact opaque cap_ handles and an optional nextCursor."},
		Safety: []string{
		...
		Example: map[string]any{"exactKind": "Deployment", "exactResource": "deployments", "apiGroup": "apps", "version": "v1", "action": "get", "namespace": "platform", "limit": 20},
	},
	{
		Name: toolDescribe, Purpose: "Load a bounded OpenAPI v3 schema fragment for one capability.", Modes: allAccessModes,
		Inputs: []ToolManualInput{
			{Name: "capabilityId", Required: true, Description: "Opaque cap_ handle returned by k8s.search."},
		...
		Safety: []string{
			...
			"fieldPath resolves local $ref, allOf, oneOf, and anyOf branches before expansion; schema expansion is capped at 10,000 nodes and reports truncation when the budget is reached.",
		},
	...
	},
	{
		...
		Inputs: []ToolManualInput{
			...
			{Name: "name", Description: "Required for get and Pod logs; for list/watch it is also used to satisfy resourceNames-restricted Roles."},
			{Name: "labelSelector", Description: "Kubernetes label selector for list or watch."},
			{Name: "fieldSelector", Description: "Kubernetes field selector for list or watch."},
			{Name: "cursor", Description: "Kubernetes continue token returned by a previous list."},
			{Name: "resourceVersion", Description: "Resource version from a list or prior watch response used to resume a watch without an event gap."},
			{Name: "limit", Description: "Requested list page size; the effective Kubernetes page is also capped at 8 items."},
			{Name: "outputMode", Description: "summary, table, or full. List and watch default to summary; get defaults to full."},
			{Name: "omitManagedFields", Description: "Defaults to true; explicitly set false only when metadata.managedFields is required."},
			{Name: "omitAnnotations", Description: "Defaults to true; explicitly set false only when annotations are required. Credential-like annotation content is still redacted."},
			{Name: "fieldPaths", Description: "Optional object-relative dot paths projected from one object or every list item; incompatible with table mode."},
			{Name: "follow", Description: "Stream Pod logs when true; this requires Dangerous mode."},
			{Name: "container", Description: "Container name for Pod logs; when omitted, get permission on the Pod is required to select its default-container annotation or first regular container."},
		...
		Steps: []string{
			...
			"Use summary or table for normal list inspection; request full or fieldPaths only when the compact result is insufficient.",
			"Use returned list cursors instead of increasing limits; resume later watches from the returned resourceVersion.",
		},
		Returns: []string{"A bounded compact summary/table, projected or full Kubernetes object/list, watch events, or Pod log lines."},
		Safety: []string{
			"Secret output follows sensitiveReads policy and is redacted by default.",
			"For list/watch with name, the server adds a matching metadata.name field selector and rejects a conflicting selector.",
			"annotations and managedFields are omitted recursively by default; credential-like annotations are redacted even when annotations are explicitly requested.",
			"follow=true is available only in Dangerous mode and remains bounded by streamTimeout, maxListItems, and maxOutputBytes.",
		},
		Example: map[string]any{"capabilityId": "<list-capability-from-k8s.search>", "namespace": "platform", "outputMode": "table", "limit": 8},
	},
	{
		...
		Inputs: []ToolManualInput{
			...
			{Name: "container", Description: "Container for exec or attach; when omitted, the default-container annotation or first regular container is selected during planning."},
		...
		Safety: []string{
			...
			"Write capabilities require get permission on the base resource because planning and commit preconditions read the target before mutation.",
